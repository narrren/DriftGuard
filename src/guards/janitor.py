...
class AWSJanitor(CloudJanitor):
    def __init__(self, credentials=None):
        ...
        if credentials and 'aws' in credentials:
...
class AzureJanitor(CloudJanitor):
    def __init__(self, credentials=None):
        try:
            ...
            # Use provided OIDC/Client Secret credentials if available
            if credentials and 'azure' in credentials:
                token_data = credentials['azure']
                access_token = token_data.get('access_token')
                if access_token:
                    print("  ✅ Using User OAuth Token for Azure")
                    self.credential = StaticTokenCredential(access_token)
            
            # Fallback to App Registration if no user token or user token failed
...
class GCPJanitor(CloudJanitor):
    def __init__(self, credentials=None):
        try:
            ...
            self.client = None
            if credentials and 'gcp' in credentials:
                token_data = credentials['gcp']
                # Check for standard OAuth2 'access_token'
                access_token = token_data.get('access_token')
                if access_token:
                    print("  ✅ Using User OAuth Token for GCP")
                    # google-auth Credentials requires 'token' arg for the access token string
                    creds = google_creds.Credentials(token=access_token)
                    project_id = os.getenv("GCP_PROJECT_ID") # Project ID usually needed
            ...
            if not self.client:
                ...
                if os.getenv("GCP_CREDENTIALS_JSON"):
                     self.client = storage.Client.from_service_account_json(os.getenv("GCP_CREDENTIALS_JSON"))
                else:
...
# 🚀 Factory & Entrypoint
# ==========================================
def scan_resources(policy_config, credentials=None):
    ...
    if 'aws' in targets:
        janitors.append(AWSJanitor(credentials))
    if 'azure' in targets:
        janitors.append(AzureJanitor(credentials))
    if 'gcp' in targets:
        janitors.append(GCPJanitor(credentials))
...
if __name__ == "__main__":
    # Test Run
    scan_resources({'target': ['aws', 'azure', 'gcp']}, credentials={})
